...
recipes$SepIngredients <- apply(recipes, 1, FUN = function(x)
        as.numeric(scan(text=substr(x["ingredient_ids"], 2, 
...
```{r}
recipes$RemainingIngr <- apply(recipes, 1, FUN = function(x) setdiff(x["SepIngredients"], 
                                                               commonIngredientsNum))
recipes$RemainingIngrNum <- apply(recipes, 1, FUN = function(x) len(x["RemainingIngr"]))
```
The results:
```{r}
hist(recipes$RemainingIngr,
     breaks = 20,
     main = "Number of ingredients in a recipe",
     xlab = "Number of ingredients",
     ylab = "Number of recipes")
